...
class trackingEvaluation(object):
    def __init__(self, t_sha, gt_path="/workspace/data", max_truncation = 0, min_height = 25, max_occlusion = 2, mail=None, cls="vehicle", eval_3diou=True, eval_2diou=False):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        filename_test_mapping = os.path.join(gt_path, 'evaluate_tracking_nia.seqmap.val')
        self.n_frames         = []
        self.sequence_name    = []
        ...
        # data and parameter
        self.gt_path           = os.path.join(gt_path, "track_label")
        self.t_sha             = t_sha
        self.t_path            = os.path.join("/workspace/OpenPCDet/output/track_result", t_sha, "data")
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                for tt in t:
                    ...
                    if ((self.cls=="vehicle" and tt.obj_type=="van") or (self.cls=="pedestrian" and tt.obj_type=="person_sitting") or tt_height<=self.min_height) and not tt.valid:
                        nignoredtracker+= 1
                        tt.ignored      = True
                        ignoredtrackers[tt.track_id] = 1
                        continue
                ...
                for gg in g:
                    if gg.tracker < 0:
                        if gg.occlusion>self.max_occlusion or gg.truncation>self.max_truncation\
                                or (self.cls=="vehicle" and gg.obj_type=="van") or (self.cls=="pedestrian" and gg.obj_type=="person_sitting"):
...
class stat:
    ...
    def update(self, data):
        ...
        self.mota_list.append(data['motp'])
    ...
    def print_summary(self, c):
        summary = ""
        
        summary += (f"evaluation {c}: average over recall").center(10,"=") + "\n"
        summary += ' MOTA \n'
...
def evaluate(mail,eval_3diou,eval_2diou):
    """
    ...
    if eval_3diou:
        mail.msg("Processing Result for NIA 3D MOT Benchmark")
    elif eval_2diou:
        mail.msg("Processing Result for NIA 2D MOT Benchmark")
    else:
        assert False, 'error'
    classes = []
    best_dict = {}
    # for c in ("vehicle", "pedestrian", "bike"):
    for c in ("vehicle", "pedestrian", "bike"):
        e = trackingEvaluation(t_sha=c, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou)
        ...
        stat_meter = stat(t_sha=c, cls=c, suffix=suffix, dump=dump)
        ...
        for threshold_tmp, recall_tmp in zip(threshold_list, recall_list):
            ...
            sys.stdout.write(
                    f"\revaluation with confidence threshold -- [Object: {c}] [Threshold: {round(threshold_tmp, 3)}, Recall: {round(recall_tmp, 3)}] [MOTA: {round(e.MOTA*100, 3)}]")
        best_dict[c] = best_mota
        print("")
        ...
        stat_meter.output()
        # summary = stat_meter.print_summary(c)
        # # stat_meter.plot()
        # mail.msg(summary)       # mail or print the summary.
        dump.close()
        
    print(f"---------Best result of MOTA---------")
    print(f"Vehicle: {round(best_dict['vehicle']*100, 3)}")
    print(f"Pedestrian: {round(best_dict['pedestrian']*100, 3)}")
    print(f"Bike: {round(best_dict['bike']*100, 3)}")
    print(f"Overall: {round((sum(best_dict.values())/ 3)*100, 3)}")
    # finish
    if len(classes)==0:
...
if __name__ == "__main__":
    ...
    # evaluate results
    success = evaluate(mail,eval_3diou,eval_2diou)