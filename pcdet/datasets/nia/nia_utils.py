...
def transform_annotations_to_nia_format(annos, map_name_to_nia=None, info_with_fakelidar=False):
    """
    Args:
        annos:
        map_name_to_nia: dict, map name to nia names (Car, Pedestrian, Cyclist)
        info_with_fakelidar:
    Returns:
    ...
    for anno in annos:
        ...
        for k in range(anno['name'].shape[0]):
            anno['name'][k] = map_name_to_nia[anno['name'][k]]
        ...
        if len(gt_boxes_lidar) > 0:
            if info_with_fakelidar:
                gt_boxes_lidar = box_utils.boxes3d_nia_fakelidar_to_lidar(gt_boxes_lidar)
